...
int printfile(int in, FILE* out, char* name);
void shuffle(unsigned char* ptr, long* lh, long* rh);
...
int main(int argc, char* argv[])
{
    ...
    if (argc < 2)
    {
        ...
        return(errors);	/* 2nd file causes segfault on FreeBSD-2.2.8.  Why? */
    }
}
...
int printfile( int in, FILE* out, char* name)
{
    ...
    while ((count = _read(in, data, 10)) > 0) {	/* 10 bytes = 2 words */
        ...
        if (j != 0) {		   /* Suppress large blocks of zeros */
            ...
            sprintf_s(line, 80, "%06o|%6o,%6o|%6o,%6o|", addr, hw[0], hw[1], hw[2], hw[3]);
            cp = line;
            ...
            *cp++ = ' ';
            for (i = 0; i < 10; i++) {
                if (i < count)
                {
                    ...
                    sprintf_s(cp, 80, "%02X", data[i]); cp += 2;
                }
                else
                {
                    *cp++ = ' '; *cp++ = ' ';
