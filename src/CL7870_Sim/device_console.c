...
void  device_console_output_cmd(unsigned __int16 device_address, unsigned __int16 cmd_value) {
	...
	((DEVICE_CONSOLE_DATA*)iop_device_buffer[device_address])->ctrl_command = cmd_value;
}
...
DWORD WINAPI device_console_worker_thread(LPVOID lpParam) {
	...
	while (iop_thread_stop_request[loc_device_addr] == 0) {
		...
		if (device_data->ctrl_command != 0) {

			cmd_type = (device_data->ctrl_command >> 14) & 0x0003;
			...
			case 1:
				...
				// --------terminate
				if ( (device_data->ctrl_command & 0x0400) != 0) {
					dev_reading = false;
					dev_writing = false;
			...
			// --------transfer initiate
			case 2:
				if (device_data->ctrl_command & 0x0800) {
					dev_reading = true;
