...
void disp_instruction_trace(FILE* io_unit) {
	...
	char heading[] = " PSW (before inst exe)                                             Act Int | Virt Addr |   Abs Addr    Opcode\n";
	...
	for (j = 0; j < 1024; j++) {
		...
		// --------show header every 20 lines after beginning
		if (j > 0 && (( j % 20 ) == 0)) {
			fprintf(io_unit, heading );
		}
