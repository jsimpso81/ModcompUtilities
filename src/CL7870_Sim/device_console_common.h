...
// -------- CONSOLE DEVICE -- VIA RAW TCP SOCKET OR VIA COM PORT

// -------- DEFINE CONSOLE DEBUG WORD
...
// -------NOTE: not all versions of console controller support all of these!!
#define ti_write				0x0800
#define ti_keyboard				0x0400
#define ti_ena_clock			0x0200
#define ti_clearbuffer			0x0100
#define ti_no_cr_lf_delay		0x0040
#define ti_no_echo				0x0010
#define ti_fwd					0x0008

// --------TCP interface...
#ifdef CONSOLE_TYPE_TCP
#define UPDATE_DATA_READY_FUNC device_console_tcp_update_data_not_ready
#define INPUT_STATUS_FUNC device_console_tcp_input_status
#define OUTPUT_DATA_FUNC device_console_tcp_output_data
#define OUTPUT_CMD_FUNC device_console_tcp_output_cmd
#define INPUT_DATA_FUNC device_console_tcp_input_data
#define MOUNT_UNIT_FUNC device_console_tcp_mount_unit
#define DISMOUNT_UNIT_FUNC device_console_tcp_dismount_unit
#define IO_THREAD_FUNC device_console_tcp_comm_worker_thread
#define WORKER_THREAD_FUNC device_console_tcp_worker_thread
#define INIT_FUNC device_console_tcp_init
#define PROC_CMD_FUNC device_console_tcp_process_command
#endif

#ifdef CONSOLE_TYPE_SERIAL
#define UPDATE_DATA_READY_FUNC device_console_serial_update_data_not_ready
#define INPUT_STATUS_FUNC device_console_serial_input_status
#define OUTPUT_DATA_FUNC device_console_serial_output_data
#define OUTPUT_CMD_FUNC device_console_serial_output_cmd
#define INPUT_DATA_FUNC device_console_serial_input_data
#define MOUNT_UNIT_FUNC device_console_serial_mount_unit
#define DISMOUNT_UNIT_FUNC device_console_serial_dismount_unit
#define IO_THREAD_FUNC device_console_serial_comm_worker_thread
#define WORKER_THREAD_FUNC device_console_serial_worker_thread
#define INIT_FUNC device_console_serial_init
#define PROC_CMD_FUNC device_console_serial_process_command
#endif



void PROC_CMD_FUNC(SIMJ_U16 loc_cmd, DEVICE_CONSOLE_DATA* device_data);

// TODO: fix strict typing errors!


// ============================================================================================================================
// --------internal function to update data_not_ready bit in status word..
// --------should be called with status locked.
void UPDATE_DATA_READY_FUNC(SIMJ_U16* loc_ctrl_status, DEVICE_CONSOLE_DATA* device_data) {
	...
	if (!device_data->write_in_progress) {
		...
		if (device_common_buffer_isempty(&device_data->in_buff)) {
			*loc_ctrl_status |= status_data_not_ready; // no data to read.
			*loc_ctrl_status &= ~status_input_ready;
		}
		// --------something to read.
		else {
			*loc_ctrl_status &= (~status_data_not_ready); // something to read.
			*loc_ctrl_status |= status_input_ready;
		}
	}

	// --------if writing and buffer space available, update status.
	else if (device_data->write_in_progress) {
		if (device_common_buffer_isfull(&device_data->out_buff)) {
			*loc_ctrl_status |= status_data_not_ready; // cant write
		}
		else {
...
// ============================================================================================================================
void  OUTPUT_DATA_FUNC(SIMJ_U16 device_address, SIMJ_U16 data_value) {

	// --------get the pointer to the data buffer
	DEVICE_CONSOLE_DATA* device_data = (DEVICE_CONSOLE_DATA*)iop_device_buffer[device_address];
	...
	device_common_buffer_put(&device_data->out_buff, junk);

	// -------- Request ownership of the resource.
	TAKE_RESOURCE(device_data->ResourceStatusUpdate);
	// -------set data not ready if needed.
	UPDATE_DATA_READY_FUNC(&device_data->ctrl_status, device_data);
...
// ============================================================================================================================
void  OUTPUT_CMD_FUNC(SIMJ_U16 device_address, SIMJ_U16 cmd_value) {

	DEVICE_CONSOLE_DATA* device_data = (DEVICE_CONSOLE_DATA*)iop_device_buffer[device_address];

	//printf("\n device_console_xxx output cmd -- called - %04x\n", cmd_value);

	// --------process the command
	PROC_CMD_FUNC(cmd_value, device_data);
...
// ============================================================================================================================
SIMJ_U16  INPUT_DATA_FUNC(SIMJ_U16 device_address) {
	...
	new_data = device_common_buffer_get(&device_data->in_buff, &ourbyte);
	...
	// fprintf(stderr," device_console_xxx input data -- called - 0x%04x, index %d, new: %s \n", ourvalue, databuffer->in_buff.last_byte_read_index, (new_data ? "New  " : "Empty"));
...
// ============================================================================================================================
SIMJ_U16  INPUT_STATUS_FUNC(SIMJ_U16 device_address) {

	DEVICE_CONSOLE_DATA* device_data = (DEVICE_CONSOLE_DATA*)iop_device_buffer[device_address];

	SIMJ_U16 loc_ctrl_status;
	//-needed??--SIMJ_U16 loc_ctrl_status1;
	...
	// --------is this needed ?
	UPDATE_DATA_READY_FUNC(&device_data->ctrl_status, device_data);
	loc_ctrl_status = device_data->ctrl_status;
	// -------- Release ownership of the resource.
	GIVE_RESOURCE(device_data->ResourceStatusUpdate);

	// printf("\n device_console_xxx input status -- called - 0x%04x\n", loc_ctrl_status);

	return loc_ctrl_status;
}
...
void MOUNT_UNIT_FUNC(SIMJ_U16 device_address, SIMJ_U16 unit, bool read_only, char* filename) {

	// --------get the pointer to the device data
	DEVICE_CONSOLE_DATA* device_data = NULL;
...
void DISMOUNT_UNIT_FUNC(SIMJ_U16 device_address, SIMJ_U16 unit) {
...
DWORD WINAPI IO_THREAD_FUNC(LPVOID lpParam) {
...
#ifdef CONSOLE_TYPE_TCP
	SOCKET loc_socket = (SOCKET)NULL;
#endif
#ifdef CONSOLE_TYPE_SERIAL
	HANDLE loc_comm_handle = NULL;
	SIMJ_U16 loc_baud = 9600;
#endif
	...
	// --------Create, listen, connect a raw TCP socket.
	// TODO: this should be in the processing loop so disconnections can be reconnected..
	open_status = device_common_raw_socket_open(device_data->ipport, &loc_socket, &last_error);

	// -------- if tcp socket was opened..
	if (open_status == 0) {

	}
	// -------- error opening TCP socket, stop thread...
	else {
	...
	// --------Open com port
	open_status = device_common_serial_open(device_data->filename, &loc_comm_handle, &last_error);

	// -------- if com port was opened..
	if (open_status == 0) {

		// --------set global com handle for other thread.
		device_data->com_handle = loc_comm_handle;

		// -------- set com port parameters
		loc_baud = device_data->ipport;
		set_param_status = device_common_serial_set_params(loc_comm_handle, &last_error, true, loc_baud);
		...
		BOOL success = SetCommTimeouts(loc_comm_handle, &timeouts);
		if (!success) {
		...
		success = SetupComm(loc_comm_handle, 16384, 50);
		if (!success) {
			printf(" *** ERROR ***  Failed to set serial port buffers\n");
		}

		// Flush away any bytes previously read or written.
		success = FlushFileBuffers(loc_comm_handle);
		if (!success) {
	...
	// TODO: Console -- THis is way to crude with too many data copies potentially high latency.  Fix.

	// -------------------------------------------------------------------------------------------

	// -------- set initial com state
	com_state = 0;
	...
	while ((iop_thread_stop_request2[loc_device_addr] == 0) && (loc_stop_request == 0)) {
		...
		case 1:
			...
			// --------if read in progress, do a read.  --- only do a read if buffer is empty.  
			// --------This way interrupts can keep up!
			...
			// if (device_common_buffer_isempty(&device_data->in_buff)) {
				...
				if (actual_read_bytes > 0) {
					...
					// --------signal data ready.
					device_data->ctrl_status &= (~status_data_not_ready);
					device_data->ctrl_status |= (status_input_ready);
					...
					if (device_data->DI_enabled) {
						cpu_request_DI(device_data->bus, device_data->pri, device_data->device_address);
						// DEBUG
						// printf("\n Console xxx - read - requested DI\n\n");
					}
					//else {
						// DEBUG
						// printf("\n Console xxx - read - DI needed, but not enabled.\n\n");
					//}
	...
	// --------try to close IO port serial or tcp socket
#ifdef CONSOLE_TYPE_TCP
	if (loc_socket != (SOCKET)NULL)
		device_common_raw_socket_close(loc_socket, &last_error);
#endif
#ifdef CONSOLE_TYPE_SERIAL
	if (loc_comm_handle != NULL)
		device_common_serial_close(loc_comm_handle, &last_error);
#endif
...
DWORD WINAPI WORKER_THREAD_FUNC(LPVOID lpParam) {
	...
	// --------MOVED IO PORT OPEN TO COM WORKER THREAD...
	...
	while (iop_thread_stop_request[loc_device_addr] == 0) {
		...
		//	PROC_CMD_FUNC( SIMJ_U16 loc_cmd, DEVICE_CONSOLE_DATA* device_data );
	...
	// --------MOVED IO DEVICE PORT CLOSE TO COM WORKER THREAD...
...
// --------data buffer after it is created.
#if CONSOLE_TYPE_TCP
void INIT_FUNC(SIMJ_U16 device_address, SIMJ_U16 bus, SIMJ_U16 prio, SIMJ_U16 dmp, SIMJ_U16 port) {
#endif
#if CONSOLE_TYPE_SERIAL
void INIT_FUNC(SIMJ_U16 device_address, SIMJ_U16 bus, SIMJ_U16 prio, SIMJ_U16 dmp,
		char* filename, SIMJ_U16 baud) {
#endif
	...
	if (device_data != NULL) {
		...
		device_data->ipport = baud;	 // tcp/udp port number for ip devices (console, async, etc., use for baud..)
		device_data->com_handle = NULL;
#endif
...
void PROC_CMD_FUNC(SIMJ_U16 loc_cmd, DEVICE_CONSOLE_DATA* device_data) {
	...
	SIMJ_U16 cmd_type = 0;
	SIMJ_U16 orig_ctrl_status = 0;	// local copy of starting controller status.
	SIMJ_U16 loc_ctrl_status = 0;	// local copy of controller status
	bool old_read = false;
	...
	// --------get internal status to work on...
	loc_ctrl_status = device_data->ctrl_status;
	orig_ctrl_status = loc_ctrl_status;
	...
	case cmd_control:
		...
		if ((loc_cmd & ctrl_terminate) != 0) {
			...
			// --------update status
			loc_ctrl_status |= (status_data_not_ready);		// set no data ready
			loc_ctrl_status &= (~status_busy);				// clear busy

			// --------generate SI
			if (!(loc_ctrl_status & status_busy)) {
...
#if CONSOLE_DEBUG >= 1
				fprintf(stderr, " Device console - TERM w/ICB cmd requested. Dev Addr %d, cmd 0x%04x\n", device_data->device_address, loc_cmd);
#endif
				msg_term_icb = true;
...
#if CONSOLE_DEBUG >= 1
				fprintf(stderr, " Device console - TERM cmd requested. Dev Addr %d, cmd 0x%04x\n", device_data->device_address, loc_cmd);
#endif
		...
		// --  	not require an SI Release command.
...
#if CONSOLE_DEBUG >= 1
			fprintf(stderr, " Device console xxx - NOOP Command. Dev Addr %d, cmd 0x%04x\n", device_data->device_address, loc_cmd);
#endif
			...
			if ((loc_ctrl_status & status_busy) == 0) {
				loc_ctrl_status &= (~status_break);		// reset break...
			...
			if (loc_cmd & ctrl_break_select) {
				...
				case ctrl_enable_brk_det:
					...
					loc_ctrl_status &= (~status_break);		// in case it is on, turn it off.
					break;
	...
	// --------update data_not_ready flag.
	UPDATE_DATA_READY_FUNC(&loc_ctrl_status, device_data);

	// --------update device status
	device_data->ctrl_status = loc_ctrl_status;
	...
	if (msg_unexpected_cmd)
		fprintf(stderr, " *** ERROR *** Device console - unexpected command.  Dev addr: %d,  cmd 0x%04x\n", device_data->device_address, loc_cmd);
