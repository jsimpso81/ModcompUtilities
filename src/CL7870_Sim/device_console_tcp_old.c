...
void device_console_tcp_update_data_not_ready(SIMJ_U16* loc_ctrl_status, DEVICE_CONSOLE_DATA* device_data) {
...
void  device_console_tcp_output_data(SIMJ_U16 device_address, SIMJ_U16 data_value) {
	...
	device_console_tcp_update_data_not_ready(&device_data->ctrl_status, device_data);
...
void  device_console_tcp_output_cmd(SIMJ_U16 device_address, SIMJ_U16 cmd_value) {
	...
	// --------process the command
	device_console_tcp_process_command(cmd_value, device_data);
			...
			if ( device_common_buffer_isempty(&device_data->in_buff)) {
				...
				if (actual_read_bytes > 0) {
					...
					if (device_data->DI_enabled) {
						cpu_request_DI(device_data->bus, device_data->pri, device_data->device_address);
						// DEBUG
						printf("\n Console TCP - read - requested DI\n\n");
					}
					else {
						// DEBUG
						printf("\n Console TCP - read - DI needed, but not enabled.\n\n");
					}
...
void device_console_tcp_process_command(SIMJ_U16 loc_cmd, DEVICE_CONSOLE_DATA* device_data) {
	...
	case cmd_control:
		...
		// --  	not require an SI Release command.
			...
			fprintf(stderr, " Device console tcp - NOOP Command. Dev Addr %d, cmd 0x%04x\n", device_data->device_address, loc_cmd);
	...
	device_console_tcp_update_data_not_ready(&loc_ctrl_status, device_data);
