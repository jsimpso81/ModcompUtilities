...
C**   COMMON /MTXCOM/ MTEXT
     ...
     2MAXTRS,HINTED,HNTLOC,KK
      COMMON /IOSCOM/ KBIN, KBOUT, DKIN, DKRAN
C
C::   DATA BLANK /' '/,CYE/'YE'/,CNO/'NO'/
C::   DATA EOF/'>$'/,CY/'Y'/,CN/'N'/
C::   DATA CPO/'PO'/,CUR/'UR'/,CWE/'WE'/,CST/'ST'/
C::   DATA CEN/'EN'/,CTE/'TE'/
C
      DEFINE FILE 2(2100,37,U,ASCVAR)
C
      CALL INIT
      CALL MAIN
      CALL EXIT
...
C
      ...
      DATA CEN/'EN'/,CTE/'TE'/
      DATA KBIN  / 5 /, KBOUT / 6 /, DKIN / 1 /, DKRAN / 2 /
C
      END
C#######################################################################
C--INITAD.FOR
...
C::   LOGICAL*1 BLKLIN, NOINPT
      LOGICAL   BLKLIN, NOINPT
      LOGICAL LMWARN,CLOSNG,PANIC,HINTED,BITSET,
     1        CLOSED,GAVEUP,SCORNG,DSEEN
C
      DIMENSION LINES(36)
      DIMENSION TRAVEL(750),TRVCON(750),TRVLOC(750)
...
C       HNTMAX IS SET TO THE MAX2 HINT NUMBER (<= HNTSIZ).  NUMBERS 1-3
...
C
 1002 READ(DKIN,1003)SECT
 1003 FORMAT(I6)
      WRITE(KBOUT,930) SECT
...
C
 1004 READ(DKIN,1005)LOC,LINES
 1005 FORMAT(1I6,36A2)
      WRITE(DKRAN'ASCVAR) LOC,LINES
      LINUSE = ASCVAR-1
...
C
 1020 OLDLOC = LOC
      IF(LINUSE.GT.FILSIZ)CALL BUG(2)
      GOTO 1004
C
...
C
1030  READ(DKIN,1032)LOC,J,NEWLOC, TK
1032  FORMAT(13I6,/,18X,10I6)
 1031 FORMAT(16I8)
      IF(LOC.EQ.-1)GOTO 1002
...
C
 ...
 1043 READ(DKIN,1041)KTAB(TABNDX),ATAB(TABNDX),A2TAB(TABNDX)
 1041 FORMAT(I6,2A2)
...
C
 ...
 1300 HINTLC(I)=0
C
      WRITE(KBOUT,931) TABNDX,TABSIZ,VCOUNT,VRBSIZ,CLSSES,CLSMAX,
...
C       0       NO DWARF STUFF YET (WAII UNTIL REACHES HALL OF MISTS)
...
C
      WRITE(KBOUT,932)
932   FORMAT(' INITIALIZATION COMPLETED.')
1     CONTINUE
C::   CALL CLOSE(1)
      RETURN
      END
...
C::   LOGICAL*1 BLKLIN, NOINPT
     ...
     1          CLOSED,GAVEUP,SCORNG,YEA,FORCED,PCT
C
      DIMENSION LINES(36)
      DIMENSION TRAVEL(750),TRVCON(750),TRVLOC(750)
      DIMENSION KTAB(300),ATAB(300),A2TAB(300)
...
C**   COMMON /MTXCOM/ MTEXT
     ...
     1CLOSNG,PANIC,CLOSED,GAVEUP,SCORNG,ODLOC,STREAM
      COMMON /MISC2/ I,RTXSIZ,CLSMAX,MAGSIZ,LOCSIZ,CTEXT,STEXT,LTEXT,
     1SECT,TRAVEL,TRVCON,TRVLOC,TRVSIZ,TABNDX,OBJ,J,K,VER,HNTSIZ,
     2MAXTRS,HINTED,HNTLOC,KK
      COMMON /IOSCOM/ KBIN, KBOUT, DKIN, DKRAN
C      
      EXTERNAL RAN
...
C
      ...
      LIQ(DUMMY)=LIQ2(MAX2(PROP(BOTTLE),-1-PROP(BOTTLE)))
      LIQLOC(LOC)=LIQ2((MOD2(COND(LOC)/2*2,8)-5)*MOD2(COND(LOC)/4,2)
     1  +1)
      BITSET(L,N)=IAND(COND(L),ISHFT(1,N)).NE.0
      FORCED(LOC)=COND(LOC).EQ.2
      DARK(DUMMY)=MOD2(COND(LOC),2).EQ.0.AND.(PROP(LAMP).EQ.0.OR.
     1.NOT.HERE(LAMP))
      PCT(N)=RAN(100).LT.N
C
...
C
 2000 IF(LOC.EQ.0)GOTO 99
      KK=STEXT(LOC)
      KENT=0
      IF(ABNUM.NE.0) KENT=MOD(ABB(LOC),ABBNUM)
...
C
 ...
 2605 WZDARK=DARK(0)
      ...
      I=RAN(1)
      CALL GETIN(WD1,WD1A,WD1X,D2,WD2A,WD2X)
C
...
C**   IF(TURNS.EQ.0.AND.WD1.EQ.CODE1('RESTO'))GOTO 8400
      TURNS=TURNS+1
...
C2609 IF(VERB.EQ.SAY.AND.WD2.NE.0)VERB=0
...
2609  IF ((I .NE. (WATER+1000) .AND. I .NE. (OIL+1000))
     ...
     2     GO TO 2610
      WD2=CPO
      WD2A=CUR
2610  IF(WD1 .EQ. CWE .AND. WD1A .EQ. CST ) IWEST = IWEST + 1
...
C
 4000 VERB=K
      SPK=ACTSPK(VERB)
      IF(WD2.NE.0.AND.VERB.NE.SAY) GO TO 2800
...
C  HIM, SO WE NEED OLDLC2, WHICH IS THE LAST PLACE HE WAS SAFE.)
    8 KK=KEY(LOC)
      ...
      OLDLOC=LOC
    9 LL=IABS(TRAVEL(KK))
      IF(MOD2(LL,1000).EQ.1.OR.MOD2(LL,1000).EQ.K)GOTO 10
...
C  IF OLDLOC HAS FORCED-MOTION.  K2 SAVES ENTRY -> FORCED LOC -> PREVIOU
   20 K=OLDLOC
      ...
      CALL RSPEAK(91)
      GOTO 2
   21 LL=MOD2((IABS(TRAVEL(KK))/1000),1000)
      IF(LL.EQ.K)GOTO 25
      IF(LL.GT.300)GOTO 22
      J=KEY(LL)
      IF(FORCED(LL).AND.MOD2((IABS(TRAVEL(J))/1000),1000).EQ.K)K2=KK
...
C  SPECIAL TESTS FOR POURING WATER OR OIL ON PLANT OR RUSTY DOOR.
 9130 IF(OBJ.EQ.BOTTLE.OR.OBJ.EQ.0)OBJ=LIQ(0)
      ...
      PROP(PLANT)=MOD2(PROP(PLANT)+2,6)
...
C  THE BOTTLE, DRINK THAT, ELSE MUST BE AT A WATER LOC, SO DRINK STREAM.
 9150 IF(OBJ.EQ.0.AND.LIQLOC(LOC).NE.WATER.AND.(LIQ(0).NE.WATER
     1.OR..NOT.HERE(BOTTLE)))GOTO 8000
      ...
      GOTO 2011
C
C  RUB.  YIELDS VARIOUS SDEDE REMARKS.
...
C  FILL.  BOTTLE MUST BE EMPTY, AND SOME LIQUID AVAILABLE.  (VASE IS NAS
 9220 IF(OBJ.EQ.VASE)GOTO 9222
      ...
      PROP(BOTTLE)=MOD2(COND(LOC),4)/2*2
      K=LIQ(0)
...
C**   IF(DEMO)GOTO 2011
 ...
 8302 FORMAT(/,' I CAN SUSPEND YOUR ADVENTURE FOR YOU SO THAT YOU CAN',
     ...
     2I3,' MINUTES BEFORE CONTINUING.')
      ...
      SETUP=-1
      R=0
       IF(WD2.NE.0) GOTO 3200
3100   WRITE(KBOUT,3250)
       READ(KBIN,3275)FNAME
3250   FORMAT(' FILENAME?')
3275   FORMAT(8A1)
       IF(FNAME(1).EQ.'    ') GOTO 3100
       GOTO 3300
3200  CALL DCODE1(WD2,FNAME(1))
      CALL DCODE1(WD2X,FNAME(6))
3300  CALL SVCOMN(.FALSE.,FNAME,CMADRS,CMSZES,OK)
CCC   IF(OK) STOP
      SETUP=2
      GO TO 2012
 8305 YEA=START(0)
...
C  GAME.
8400  IF(WD2.NE.0) GOTO 3400
3500  WRITE(KBOUT,3250)
      READ(KBIN,3275)FNAME
      IF(FNAME(1).EQ.'    ') GOTO 3500
      GOTO 3600
3400  CALL DCODE1(WD2,FNAME(1))
      CALL DCODE1(WD2X,FNAME(6))
3600  CALL LDCOMN(.FALSE.,FNAME,CMADRS,CMSZES,OK)
      IF(OK) GO TO 8500
      WRITE(KBOUT,1112)
1112  FORMAT(" YOU ARE BEING GIVEN A FRESH GAME INSTEAD")
      GO TO 1111
8500  CONTINUE
1111  CONTINUE
      RETURN
...
40010 HINTLC(HINT)=0
      ...
      WRITE(KBOUT,40012)HINTS(HINT,2)
...
C  THAT SHOULD BE GOOD ENOUGH.  LET'S TELL HIM ALL ABOUT IT.
      WRITE(KBOUT,20100)SCORE,MXSCOR,TURNS
...
C
      ...
      COMMON /IOSCOM/ KBIN, KBOUT, DKIN, DKRAN
      IF(N.EQ.0)RETURN
      READ(DKRAN'N) LOC, LINES
      IF ( LINES(1) .EQ. EOF ) RETURN
      IF ( BLKLIN .AND. NOINPT ) WRITE(KBOUT,900)
  900 FORMAT(1X)
      NOINPT = .TRUE.
1     OLDLOC = LOC
      DO 3  I=1,36
      L = 37 - I
      IF ( LINES(L) .NE. BLANK ) GO TO 5
3     CONTINUE
5     WRITE(KBOUT,2) ( LINES(I),I=1,L )
2     FORMAT(1X,36A2)
      READ(DKRAN'ASCVAR) LOC, LINES
...
C      THE FOLLOWING IS "TO KEEP FORTRAN HAPPY!"
 1999 RETURN
      END
...
C
      ...
      IF (ATAB(I).EQ.ID1.AND.A2TAB(I).EQ.ID2) GO TO 3
    1 CONTINUE
      CALL BUG(21)
C
    2 V=-1
...
C
    ...
    2 IF(WHERE.LE.0)RETURN
      ...
      END
C#######################################################################
C  UTILITY ROUTINES (RAN, DATIME, BUG)
      SUBROUTINE BUG(NUM)
      IMPLICIT INTEGER  (A-Z)
      COMMON /IOSCOM/ KBIN, KBOUT, DKIN, DKRAN
...
C
      ...
      COMMON /RANCOM/ R
      IF(R.NE.0) GO TO 1
      CALL DATIME(DUM1,DUM2)
      TEMP=DUM2*2+1
      R=18*TEMP+5
1     R=R*261
      IF(R.LT.1) R=R+2*16384
      TEMP=32767/RANGE
      RAN=R/TEMP
      RND=RAN                                                            JAS
      RETURN
...
C
C  D IS SET TO THE NUMBER OF DAYS SINCE 07/01/77 (A SATURDAY)
C  T IS SET TO THE NUMBER OF MINUTES PAST MIDNIGHT.
C
      IMPLICIT INTEGER*4 (A-Z)
      DIMENSION J(8),MONTH(12)
      DATA MONTH /0,31,59,90,120,151,181,212,243,273,304,334/
      CALL TIMDT4(J,TEST,1)
      T=J(1)
      CALL TIMDT4(J,TEST,0)
      YEAR=J(7)
      DAY=MONTH(J(5))+J(6)
      D=DAY+365*(YEAR-1977)+(YEAR-1977)/4+181
      RETURN
      END
